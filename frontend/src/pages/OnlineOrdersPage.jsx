          ...
          <div className="flex gap-1 bg-white dark:bg-slate-900 rounded-lg border border-border dark:border-slate-800 p-1">
            {['SWIGGY', 'ZOMATO'].map(p => (
              <button key={p} onClick={() => setPlatform(p)}
                className={`px-4 py-1.5 rounded-md text-xs font-medium transition-colors ${platform === p ? (p === 'SWIGGY' ? 'bg-orange-500 text-white' : 'bg-red-500 text-white') : 'text-text-secondary dark:text-slate-400 hover:bg-surface dark:hover:bg-slate-800'}`}>{p}</button>
            ))}
          </div>
          {canCreate && (
            <button onClick={() => { setForm(f => ({ ...f, platform })); setAddModal(true); }}
              className="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-light transition-colors cursor-pointer">
        ...
        <div className="space-y-4 max-h-[60vh] overflow-auto">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.externalOrderId} onChange={e => setForm(f => ({ ...f, externalOrderId: e.target.value }))}
                className="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-border dark:border-slate-700 text-slate-900 dark:text-slate-100 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" />
            </div>
            <div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={addItem} className="text-xs text-primary dark:text-blue-400 hover:underline flex items-center gap-1"><Plus size={14} /> Add</button>
            </div>
            {form.items.map((item, i) => (
              <div key={i} className="flex gap-2 mb-2">
                <input placeholder="Item name" value={item.name} onChange={e => updateItem(i, 'name', e.target.value)}
                  className="flex-1 px-3 py-2 bg-white dark:bg-slate-800 border border-border dark:border-slate-700 text-slate-900 dark:text-slate-100 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" />
                <input type="number" placeholder="Qty" value={item.quantity} onChange={e => updateItem(i, 'quantity', e.target.value)} min="1"
                  className="w-16 px-3 py-2 bg-white dark:bg-slate-800 border border-border dark:border-slate-700 text-slate-900 dark:text-slate-100 rounded-lg text-sm text-center focus:outline-none focus:ring-2 focus:ring-primary/20" />
                <input type="number" placeholder="Price" value={item.price} onChange={e => updateItem(i, 'price', e.target.value)}
                  className="w-24 px-3 py-2 bg-white dark:bg-slate-800 border border-border dark:border-slate-700 text-slate-900 dark:text-slate-100 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary/20" />
                {form.items.length > 1 && <button onClick={() => removeItem(i)} className="text-danger"><X size={16} /></button>}
              </div>
            ))}
          </div>
          ...
          <button onClick={submitOrder} className="w-full py-2.5 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-light transition-colors">Save Order</button>
        </div>
      </Modal>
              ...
              <div className="flex gap-2">
                {statusFlow.slice(statusFlow.indexOf(detailModal.status) + 1).map(s => (
                  <button key={s} onClick={() => updateStatus(detailModal.id, s)}
                    className="flex-1 py-2 bg-primary/10 text-primary dark:text-blue-400 rounded-lg text-sm font-medium hover:bg-primary/20 dark:hover:bg-primary/30 transition-colors cursor-pointer">→ {s}</button>
                ))}
              </div>
            )}
          </div>
